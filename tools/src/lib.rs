...
use svg::node::element::{path::Data, Line, Path, Rectangle};
    ...
    for d in 0.. {
        ...
        for &(a, b) in &out[d as usize] {
            ...
            if worker[b] != !0 {
                return (
                    0,
                    format!("Task {} has been assigned multiple times", b + 1),
                );
                ...
                return (
                    0,
                    format!("Dependent tasks of task {} have not been completed", b + 1),
                );
            }
            worker[b] = a;
...
pub fn get_colors(input: &Input, out: &Output, ty: i32) -> Vec<String> {
    if ty == 0 {
        (0..input.N)
            .map(|i| {
                color(
                    ((input.d[i].iter().map(|&x| x * x).sum::<i32>() as f64).sqrt() - 10.0) / 30.0,
                )
            })
            .collect()
    } else if ty == 1 {
        (0..input.N)
            .map(|i| color(i as f64 / input.N as f64))
            .collect()
    } else if ty == 2 {
        ...
        (0..input.N)
            .map(|i| color((diff[i] + 20.0).max(0.0).min(40.0) / 40.0))
            .collect()
    } else {
...
pub fn vis(input: &Input, out: &Output, d: usize, ty: i32) -> (i64, String, String) {
    ...
    for d in 0..out.len().min(d) {
        for &(a, b) in &out[d] {
            doc = doc.add(
                rect(a * W, 20 + W + d, W, input.t[b][a] as usize, &colors[b])
                    .set("stroke", "black")
                    .set("stroke-width", 1),
            );
        }
